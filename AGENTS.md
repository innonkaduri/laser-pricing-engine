...
בזבוז מדורג → רצפת פלטה → מחיר. **נפרד ברובו מה-CRM** (פרויקט
"CRM ימיש חדש") — שני חיבורים בלבד, שניהם דרך מפתחות ב-`/admin`:
`SERVICE_TOKEN` (ה-CRM קורא ל-`POST /api/quote` ומקבל `quote:use`
בלבד — **חוּוט ונבדק קצה-לקצה ב-1.9.2026**, `/api/prices` נשאר 403
גם איתו) ו-**webhook יוצא** (1.9.2026, `CRM_WEBHOOK_URL`/`_SECRET`):
כל הזמנה חדשה נשלחת ל-CRM עם פירוק המחיר המלא וקובצי החיתוך של
הפריטים. **חילוץ ה-DXF שמעלה לקוח נשאר `ezdxf` ומקומי לגמרי ונמחק
מיד** — הקובץ שנשלח ל-CRM נבנה מחדש מהמפרט השמור של ההזמנה, לא
הקובץ שהועלה, ויוצא אך ורק ליעד הזה, אם הוגדר.
