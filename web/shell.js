...
export async function mountSidebar(active = location.pathname) {
  ...
  const items = LINKS.filter((link) => !link.cap || caps.has(link.cap))
    .map((link) => {
      ...
      const badge = link.cart
        ...
        : link.shop
          ? '<em class="sb-count" id="sb-shop-count" hidden>0</em>'
  ...
  nav.innerHTML = `
    ...
    <div class="sb-foot">${who}
      <!-- **תקנון ונגישות בכל מסך, ולא בדף אחד.** בישראל זו חובה,
           והדרך היחידה שהיא באמת מתקיימת היא שהיא יושבת ברכיב
           המשותף — קישור שמועתק ידנית מגיע לשני מסכים ונשכח בשאר. -->
      <div class="sb-legal">
        <a href="/terms">תקנון</a><a href="/accessibility">נגישות</a><a href="/contact">צור קשר</a>
      </div>
    </div>`;
...
/* תג ההזמנות החדשות — **רק למי שרשאי לראות אותן.** בקשה שמחזירה
   403 בכל טעינת מסך היא רעש בלוג ולא מידע. */
export async function refreshShopBadge(caps) {
  const tag = document.getElementById('sb-shop-count');
  if (!tag || !caps || !caps.has('quote:use')) return 0;
  try {
    const response = await fetch('/api/shop/orders/new');
