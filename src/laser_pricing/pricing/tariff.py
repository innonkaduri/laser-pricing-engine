...
class Tariff:
    ...
    """
    source: str = "<inline>"
    """מאיפה נטענה הטבלה — שם הקובץ. **לא** מי קבע את המחירים."""

    price_origin: str = ""
    """מי קבע את המספרים, ובאיזו רמת ודאות. ריק = לא הוצהר.

    **נולד מהכרעת ינון ב-25.8.2026:** "תבדוק את המחירים בשוק ותמלא 10
    אחוז מעל". כלומר הטבלה מתמלאת בהערכת שוק ולא במחירי אבא — וזה
    חייב להיות גלוי, אחרת הצעה שנשענת על הערכה נראית זהה להצעה
    שנשענת על מחיר אמיתי. השדה הזה נוסע עם הטבלה לכל מקום שהיא
    מגיעה אליו: `/health`, `/api/config`, וכל הצעה שיוצאת.

    הוא **אינו** מוגבל להערכות: כשאבא ימלא, הוא כותב כאן שהמחירים
    שלו — וזו ההצהרה שמסירה את האזהרה.
    """

    price_low_confidence: list[str] = field(default_factory=list)
    """שמות שדות שהערך בהם חלש במיוחד, גם ביחס לשאר.

    רשימה נפרדת ולא הערה חופשית, כי "הכל הערכה" ו"הכיפוף במיוחד
    ניחוש" הם שני דברים שונים, ומי שמאשר הצעה צריך להבחין ביניהם.
    """
    ...
    def rate_for(self, material_key: str, thickness_mm: float) -> MaterialRate:
        ...
        # **שורה שקיימת וריקה היא לא מחיר אפס — היא חוסר מחיר.**
        # נמצא במדידה ב-25.8.2026, אחרי מילוי חלקי של הטבלה: נירוסטה
        # 3 מ"מ (שורה בלי מחירים) החזירה **413 ₪** ולא 0, כי מינימום
        # ההזמנה החדש רץ על סכום אפס והרים אותו. כלומר הוספת המינימום
        # הפכה "תקלה גלויה" ל"מחיר קטן וסביר למראה" — בדיוק צורת הכשל
        # המסוכנת: אפס גורם למישהו להתלונן, 413 יוצא ללקוח.
        #
        # התנאי הוא `has_any_prices` ולא "תמיד": כשכל הטבלה ריקה,
        # המערכת כולה מוכרזת לא-מוכנה ומחזירה 0 עם אזהרות בקול — זו
        # ההתנהגות שאיפשרה לבנות בלי להמתין לאבא, והיא נשארת.
        if self.has_any_prices and rate.plate_price <= 0 and rate.cut_rate_per_m <= 0:
            raise MissingTariffError(
                f'ל{rate.material_name} בעובי {thickness_mm} מ"מ אין מחיר בטבלה, '
                f"בעוד שלחומרים אחרים יש. הוסף מחיר פלטה ומחיר חיתוך לשורה — "
                f"לא ננחש, ולא נחזיר מספר חלקי."
            )
        return rate

    @property
    def has_any_prices(self) -> bool:
        """האם מישהו כבר התחיל למלא. מבחין בין "טבלה ריקה" ל"שורה ריקה"."""
        return any(r.plate_price > 0 or r.cut_rate_per_m > 0 for r in self.rates.values())
