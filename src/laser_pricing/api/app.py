...
**מ-30.8.2026 גם `POST /api/cart` צורך מהדלי הזה.** מסך המוצר הפסיק
להציג "חשב מחיר" — המחיר עבר לעגלה בלבד — ולכן "הוסף לעגלה" הוא
עכשיו נקודת הכניסה למנוע במקום הכפתור שנעלם. אותו דלי, לא דלי נפרד:
מי שמעלה DXF וגם מוסיף מוצרי קטלוג לעגלה חולק תקרה אחת.
"""
...
def cart_add(body: CartAddRequest, request: Request) -> dict:
    # **30.8.2026: מסך המוצר הפסיק להציע "חשב מחיר".** המחיר מוצג רק
    # בעגלה, ולכן "הוסף לעגלה" הוא עכשיו הפעולה שמזינה את מנוע
    # התמחור — בדיוק כמו `/api/upload` ו-`/api/manual` לפניה. בלי
    # השומר כאן, העגלה הופכת למד-מחיר בלי תקרה: מוסיפים פריט, פותחים
    # `GET /api/cart` (שאינו מוגבל בעצמו — צפייה חוזרת באותה עגלה לא
    # חושפת מידע חדש), מסירים, מוסיפים אחר — וחוזר חלילה בלי הגבלה,
    # בדיוק ההתקפה שתועדה ב-`DECISIONS.md` (23.8.2026).
    _guard_public_engine(request)
    username = _cart_owner(request)
